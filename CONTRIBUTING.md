# Contributing to Just Firebase

Thank you for your interest in contributing to **Just Firebase**! We welcome all contributions — bug fixes, new features, documentation improvements, and more.
...
2. **Clone** your fork locally:
   ```bash
   git clone https://github.com/just-unknown-dev/just-firebase.git
   cd just-firebase
   ```
...
- Flutter/Dart SDK versions (`flutter --version`) and Firebase SDK versions.
...
- Check that the feature aligns with the package's scope (Firebase integration — Auth, Firestore, Analytics, and Remote Config).
...
The repository is a **Flutter package**. You can develop and test it standalone or with the companion app at the workspace root.

```bash
# Run unit tests (from packages/just_firebase)
flutter test
...
- Keep subsystems decoupled — changes to one Firebase integration module should not break others.
...
**Examples:**
```
feat(auth): add Apple sign-in provider support
fix(firestore): correct offline cache invalidation on reconnect
...
By contributing to Just Firebase you agree that your contributions will be licensed under the [BSD-3-Clause License](LICENSE).
